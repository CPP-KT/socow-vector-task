...
- Конструктор копирования и оператор копирующего присваивания должны работать за `O(SMALL_SIZE)`, а не за `O(size)`.
- Конструктор перемещения и оператор перемещающего присваивания должны работать за `O(SMALL_SIZE)`.
...
- `insert(const_iterator pos, const T& value), insert(const_iterator pos, T&& value)` &mdash; вставить элемент перед `pos`;
